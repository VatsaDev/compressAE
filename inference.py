...
def preprocess_image(image_path, patch_size):
    ...
    # Reverted to RGB tensor conversion instead of Lab
    transform = transforms.ToTensor()
    patches = [transform(img_resized.crop((j, i, j + patch_size, i + patch_size))).unsqueeze(0) for i in range(0, new_h, patch_size) for j in range(0, new_w, patch_size)]
...
with torch.no_grad():
    # Reconstructed directly in RGB space (no lab_to_rgb)
    reconstructed_patches_rgb = model.dec(latents)
